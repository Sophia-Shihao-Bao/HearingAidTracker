...
/// Minimal BLE manager for watchOS mirroring the iOS behavior (HM-10/BT05 style UART)
final class WatchBLEManager: NSObject, ObservableObject {
    ...
    // BLE constants (HM-10 / BT05 UART-like)
    private let TARGET_NAME_HINT = "BT05"          // change if needed
    ...
    func ledOn()  { writeLED(byte: 0x31) } // '1'
    func ledOff() { writeLED(byte: 0x30) } // '0'
...
extension WatchBLEManager: CBCentralManagerDelegate, CBPeripheralDelegate {
    ...
    func centralManager(_ central: CBCentralManager, didFailToConnect peripheral: CBPeripheral, error: Error?) {
        ...
        statusHint = "Failed to connect: \(error?.localizedDescription ?? "unknown")"
    }
    ...
    func peripheral(_ peripheral: CBPeripheral, didDiscoverServices error: Error?) {
        if let error = error {
            statusHint = "Service error: \(error.localizedDescription)"
            return
        }
    ...
    func peripheral(_ peripheral: CBPeripheral, didDiscoverCharacteristicsFor service: CBService, error: Error?) {
        if let error = error {
            statusHint = "Char error: \(error.localizedDescription)"
            return
        }
    ...
    func peripheral(_ peripheral: CBPeripheral, didUpdateValueFor characteristic: CBCharacteristic, error: Error?) {
        if let error = error {
            statusHint = "Update error: \(error.localizedDescription)"
            return
        }
